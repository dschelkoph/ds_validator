...
from ds_validator.decorators import bundle
...
ComboValidator: TypeAlias = Annotated[
    torch.Tensor,
    bundle(
        tensor_shape(shape=("nodes", EMBEDDING_DIMENSIONS, range(1, 5), "x", "x", None)),
        tensor_dtype(data_type=torch.int64),
        tensor_device(device=torch.device("cpu")),
    ),
]
combo_tensor_validator = ds_type_adapter(ComboValidator)

DTypeValidator: TypeAlias = Annotated[
    torch.Tensor, tensor_dtype(data_type={torch.uint64, torch.int64})
]
dtype_tensor_validator = ds_type_adapter(DTypeValidator)

DeviceValidator: TypeAlias = Annotated[
    torch.Tensor, tensor_device(device=torch.device("cuda:0"), match_index=True)
]
...
def test_invalid_shape_range():
    with pytest.raises(ValidationError):
        tensor_shape(shape=(range(-1, -4), 1))


@pytest.mark.skip()
def test_invalid_shape_int():
    with pytest.raises(ValidationError):
        tensor_shape(shape=(1, -3))
